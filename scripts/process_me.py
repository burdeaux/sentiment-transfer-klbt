"""
This script processes the yelp_long dataset
...
class MonoCsvData(object):
    ...
    def __init__(self, fname, label='label', max_length=None, vocab=None):
        super(MonoCsvData, self).__init__() #label
...
if __name__ == '__main__':
    # save_dir = "../data/yelp_long"
    # if not os.path.exists(save_dir):
    #     os.makedirs(save_dir)

    yelp_dir = "data/yelp_long"
    yelp_csv_file='senti_csv'
    file_name=os.path.join(yelp_dir,yelp_csv_file)
    data = MonoCsvData(file_name,'label')
    #write vocab
    with open(os.path.join(yelp_dir, "text.vocab"), "w") as fout:
        vocab = data.vocab.id2word
        for i in range(len(data.vocab)):
            fout.write("{}\n".format(vocab[i]))
    #write attr vocab(label value) 
    with open(os.path.join(yelp_dir, "attr.vocab"), "w") as fout:
        fout.write("negative\npositive\n")
        
        
    ftrain_t = open(os.path.join(yelp_dir, "long_yelp.txt"), "w")
    ftrain_a = open(os.path.join(yelp_dir, "long_yelp.attr"), "w")

    for line,label in zip(data.data,data.labels):
        ftrain_t.write(line)
        if label==0:
