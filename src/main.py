...
#for exp num
parser.add_argument("--exp_num",type=float,default=1)
args = parser.parse_args()

if args.bpe_ngram: args.n = None

if args.output_dir == "":
    #gs_{t}if gumbel_softmax or {t} for sampling
    ...
    #fix the output_dir
    args.output_dir = "outputs_transformer1{}{}{}/{}_wd{}_wb{}_ws{}_an{}_pool{}_klw{}_lr{}_{}{}{}{}{}{}{}{}/".format(args.exp_num,args.dataset, dual_str, args.dataset,
...
#load lm_model in config.config_yelp 
...
def eval(model, classifier, data, crit, step, hparams, eval_bleu=False,
  ...
  while True:
    ...
    valid_acc += val_acc #noise accuracy
    valid_trans_acc += val_transfer_acc #transfer accuracy
  ...
  log_string += " bt_ppl={0:<6.2f}".format(bt_ppl)#total loss for back-translation output
  log_string += " n_ppl={0:<6.2f}".format(noise_ppl)#total loss for noise output
  log_string += " n_acc={0:<5.4f}".format(valid_acc / valid_words)#noise accuracy with original input(only noise model)
  log_string += " bt_acc={0:<5.4f}".format(valid_trans_acc / valid_words)#bt_accracy with original input
...
def train():
  ...
  hparams.gs_temp = 1.
  #save hparams for lm training if need
  # extra_file_for_lm='/home/rtx2070s/exp/deep-latent-sequence-model/pretrained_lm'
  # torch.save(hparams, os.path.join(extra_file_for_lm, "hparams_20.pt"))
  ...
  while True:
    ...
    if eval_now:
      ...
      based_on_bleu = False #based on bleu
