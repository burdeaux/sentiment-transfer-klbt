...
class Seq2Seq(nn.Module):
  ...
  def set_lm(self):
    if self.hparams.lm:
      ...
      print(self.hparams.src_vocab_size)
      print('this is set lm',self.LM0)
  ...
  def forward(self, x_train, x_mask, x_len, x_pos_emb_idxs, y_train, y_mask,
    ...
    if self.hparams.lm:
      ...
      y_sampled_reorder = torch.index_select(y_sampled, 0, org_index_lm) #sample reorder with org_index
      # E_{x ~ q(z|x, y)}[log p(z|y)]
      #x_trans_lm:generate sentence token
      #y_sampled_reorder:target attr(flip attr)
      log_prior = self.log_prior(x_trans_lm, x_trans_mask_lm, x_trans_len_lm, y_sampled_reorder)
      
      # add cls control 
      # fix_clssifier = 
      # dis_control_logits = fix_classifier.forward()
      # dis_control_logits = dis_control_logits.view(-1,hparams.trg_vocab_size)


      # KL = E_{x ~ q(z|x, y)}[log q(z|x, y) - log p(z|y)]
      if self.hparams.dual:
        KL_loss = 0. - log_prior
        # if self.hparams.dis_control:
        #   KL_loss=
      else:
  ...
  def get_translations(self, x_train, x_mask, x_len, y_sampled, y_sampled_mask, y_sampled_len, temperature=None):
    ...
    #translated_list with bos and eos token
    ...
    #translated_x sorted by length long to short
  ...
  def translate(self, x_train, x_mask, x_len, y, y_mask, y_len, max_len=100, beam_size=2, poly_norm_m=0, temperature=None, sampling=True):
    if sampling:
        hyps = self.sampling_translate(x_train, x_mask, x_len, y, y_mask, y_len, max_len=max_len, temperature=temperature)
        return hyps #hyp is decoded_batch list
